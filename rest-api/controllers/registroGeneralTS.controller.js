...
const postRegistroTS = async (req, res) => {
  const registroToInsert = req.body;
  const variables = await getAllVariables();
  ...
  const nuevoMetaData = registroToInsert.metaData.filter(item => nombresSet.has(item.n));
  ...
  const existeDocumentos = await RegistroGeneralts.find({fecha_lectura: nuevoregistroToInsert.fecha_lectura});
    ...
    if (existeDocumento) {
      ...
      existeDocumento.metaData.forEach((existeItem, existeIndex) => {
        nuevoregistroToInsert.metaData.forEach(nuevoItem => {
          ...
          if (existeItem.id_variable.toString() === nuevoItem.id_variable.toString()) {
      ...
      for (const metaDatoExiste of existeDocumento.metaData) {
        ...
        for (const metaDatoNuevo of nuevoregistroToInsert.metaData) {
          ...
          if (metaDatoExiste.id_variable.toString() === metaDatoNuevo.id_variable.toString()) {
  ...
  nuevoregistroToInsert.metaData = nuevoregistroToInsert.metaData.filter(metaDato => {
  ...
  for (let i = 0; i < nuevoregistroToInsert.metaData.length; i++) {
    ...
    const foundVariable = variables.find(elemento => elemento._id == nuevoregistroToInsert.metaData[i].id_variable );

    //---- Variable sin datos anterior. Se almacena si aplicar ningun filtro.-
    if( lastRegistros.find(registro => registro.id_variable == nuevoregistroToInsert.metaData[i].id_variable) === undefined){
      continue;
    }
    ...
    switch(foundVariable.tipo_trigger){
      case 'cambio-valor':
        try{
          let valor_lasRegistros = lastRegistros.find(registro => registro.id_variable == nuevoregistroToInsert.metaData[i].id_variable).v;
          let valor_nuevoregistroToInsert = nuevoregistroToInsert.metaData[i].datos;
          ...
          if( Math.abs(valor_lasRegistros - valor_nuevoregistroToInsert) >= trigger_valor){
      ...
      case "cambio-porcentaje":
        try{ 
          ...
          if( valor_lasRegistros != 0){
            valor_porcentaje = Math.abs( (valor_lasRegistros - valor_nuevoregistroToInsert) / valor_lasRegistros) * 100;  
          }
          /*
      ...
      case "cambio-tiempo":
          try{
            let fecha_lasRegistros = new Date(lastRegistros.find(registro => registro.id_variable == nuevoregistroToInsert.metaData[i].id_variable).fl).getTime();
            let fecha_nuevoregistroToInsert = new Date (nuevoregistroToInsert.fecha_lectura).getTime();
            let trigger_valor = foundVariable.trigger_valor;
            ...
            if( Math.abs(fecha_lasRegistros - fecha_nuevoregistroToInsert) >= trigger_valor){
